...
// Receives the post already fetched on the server (see page.jsx) so the full
// article is present in the initial HTML — crawlers no longer see a spinner.
// The mount-time refetch keeps view_count incrementing per real visit and picks
// up any edit made since the 5-min-cached server copy.
export default function BlogPostView({ initialPost, slug }) {
  const { lang }  = useLang();
  const hi        = lang === 'hi';
    ...
    fetch(`/api/public/blog/${slug}`)
      ...
      .then(d => {
        if (!d.success || !d.data?.post) {
          // Only fall to "not found" if the server had nothing either — never
          // hide an SSR'd post because of a transient client-side error.
          if (!initialPost) setNotFound(true);
        } else {
          setPost(d.data.post);
          setNotFound(false);
        }
      })
      .catch(() => { /* keep the server-rendered copy */ });
