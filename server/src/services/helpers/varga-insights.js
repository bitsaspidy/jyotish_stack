...
function analyzeOnePlanet(planet, pd, ascNum, slug) {
  ...
  if (house === 1 || house === 5 || house === 9) {
    positives_en.push(`${planet} in a Trikona house (${house}) gives natural support to ${planetRole} — results arrive with relatively less effort.`);
    positives_hi.push(`${P_HI[planet] || planet} त्रिकोण भाव (${house}) में है — ${karakatva.hi} के लिए स्वाभाविक समर्थन।`);
  }
  ...
  if (house === 10 && isPositive) {
    positives_en.push(`H10 gives ${planet} visibility and social recognition in ${planetRole}. This placement is career and reputation building.`);
    positives_hi.push(`10वें भाव में ${P_HI[planet] || planet} को ${houseDomain} में सामाजिक पहचान मिलती है।`);
  }
