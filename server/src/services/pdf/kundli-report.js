...
function buildKundliReport(profile, chart, extras = {}) {
  ...
  r.gap(10);
  r.heading('Ganesh Gayatri Mantra — Opening Invocation');
  r.para('An extremely powerful and sacred mantra that bestows knowledge, wisdom, and concentration. Chant 9 times before beginning any new work, Puja, or meditation session.', { size: 7.8, color: MUTED });
  r.gap(6);
  r.ensure(56);
  r.d.rect(M, r.y, W, 52, CARD2);
  r.d.rect(M, r.y, W, 2, GOLD);
  r.d.text(M + 14, r.y + 8, 'Mantra (x 9 times):', { size: 7.5, bold: true, color: GOLD });
  r.d.text(M + 14, r.y + 20, 'Om Ekadantaya Vidmahe Vakratundaya Dhimahi Tanno Dantih Prachodayat', { size: 9, bold: true, color: IVORY });
  r.d.text(M + 14, r.y + 34, 'Sanskrit: Om Ekadantaya Vidmahe Vakratundaya Dhimahi Tanno Dantih Prachodayat ||', { size: 7.5, color: GOLD2 });
  r.y += 56;
  r.gap(4);
  r.sub('Meaning');
  r.para('Ekadantaya Vidmahe: We meditate on the one-tusked Lord (Ganesha).', { size: 7.8, color: MUTED });
  r.para('Vakratundaya Dhimahi: We meditate upon Him with the curved trunk.', { size: 7.8, color: MUTED });
  r.para('Tanno Dantih Prachodayat: May He with the tusk enlighten our intellect on the path of wisdom and knowledge.', { size: 7.8, color: MUTED });
  r.gap(6);
  r.sub('Benefits');
